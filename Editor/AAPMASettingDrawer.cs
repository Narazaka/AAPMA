...
namespace Narazaka.Unity.AAPMA.Editor
{
    ...
    class AAPMASettingDrawer : UnityEditor.PropertyDrawer
    {
        ...
        class DrawerBase
        {
            ...
            protected string Input1Name => string.IsNullOrEmpty(_input1.Property.stringValue) ? "Input1" : _input1.Property.stringValue;
            protected string Input2Name => string.IsNullOrEmpty(_input2.Property.stringValue) ? "Input2" : _input2.Property.stringValue;
            protected string OutputName => string.IsNullOrEmpty(_output.Property.stringValue) ? "Output" : _output.Property.stringValue;
        }
        ...
        class Drawer : DrawerBase
        {
            ...
            public void OnGUI()
            {
                EditorGUI.PropertyField(line, _type, label);
                NextLine();
            ...
            void Addition()
            {
                EditorGUI.HelpBox(line, $"{OutputName} = {Input1Name} + {Input2Name}", MessageType.None);
                NextLine();
                DrawUse1D();
                if (Use1D)
                {
            ...
            void Division()
            {
                EditorGUI.HelpBox(line, $"{OutputName} = {_output.Max.floatValue} / (1 + {Input1Name})", MessageType.None);
                NextLine();
                ParamPositive(_input1);
                ZeroMax(_output);
            }
            ...
            void DrawUse1D()
            {
                EditorGUI.PropertyField(line, _use1D);
                NextLine();
                EditorGUI.HelpBox(line, "Use 1D is more flexible and less precise.", MessageType.Info);
                NextLine();
            }

            void Param(AAPParameterProperty property)
            {
                EditorGUI.PropertyField(line, property.Property, new GUIContent(property.Root.displayName));
                NextLine();
            }
            ...
            void MinMax(AAPParameterProperty property)
            {
                ...
                EditorGUI.PropertyField(rect, property.Min);
                rect.x += rect.width + 2;
                EditorGUI.PropertyField(rect, property.Max);
            ...
            void ZeroMax(AAPParameterProperty property)
            {
                ...
                EditorGUI.FloatField(rect, new GUIContent(property.Min.displayName), 0);
